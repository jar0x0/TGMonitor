...
- 结构化日志输出（JSON 格式）
- 异步安全，不阻塞事件循环
- 日志风格参考 bot 项目的 `bot/src/utils/logger.ts`：分级、文件轮转、独立错误日志

#### 为什么用 pydantic 做数据实体？

参考 bot 项目中 `bot/src/types/product.ts` 使用 class 定义数据实体的模式。Python 中 pydantic 是等价的最佳实践：
...
|------|-------------|---------------|------|
| **数据实体层 (Models)** | `bot/src/types/product.ts` | `src/models/*.py` | 定义数据结构（pydantic Model），字段映射，序列化/反序列化 |
| **业务逻辑层 (Services)** | `bot/src/services/productService.ts` | `src/services/*.py` | 业务逻辑处理，Redis 缓存管理，调用 Repository |
| **数据库操作层 (Repositories)** | `bot/src/repositories/productRepository.ts` | `src/repositories/*.py` | 纯数据库 CRUD 操作，SQL 语句，事务管理 |
...
参照 `bot/src/services/productService.ts` 的 Redis + MySQL 两级存储模式：
...
│   │   ├── database.py                    # MySQL 连接池（参照 bot/src/config/database.ts）
│   │   ├── redis_client.py                # Redis 客户端管理（参照 bot/src/config/redisClient.ts）
│   │   └── accounts.py                    # 多账号配置加载
│   │
│   ├── models/                            # 数据实体层（参照 bot/src/types/product.ts，Python 中 types 是标准库模块，故命名为 models）
│   │   ├── __init__.py
...
│   │
│   ├── services/                          # 业务逻辑层（参照 bot/src/services/productService.ts）
│   │   ├── __init__.py
...
│   │
│   ├── repositories/                      # 数据库操作层（参照 bot/src/repositories/productRepository.ts）
│   │   ├── __init__.py
...
│   └── utils/                             # 工具模块
│       ├── __init__.py
│       └── logger.py                      # 日志配置（参照 bot/src/utils/logger.ts 的分级模式）
│
...
**参照**: `bot/src/types/product.ts` 中 `Product` 类的定义方式
...
**参照**: `bot/src/config/redisClient.ts` 中的 `RedisClientManager` 单例模式
...
**参照**: `bot/src/utils/logger.ts` 的日志分级和文件轮转模式
...
- [ ] **步骤 1.2**: 实现配置模块
  ...
  - `config/database.py` — MySQL 异步连接池（参照 `bot/src/config/database.ts`）
  - `config/redis_client.py` — Redis 客户端管理（参照 `bot/src/config/redisClient.ts`）

- [ ] **步骤 1.3**: 实现日志系统
  - `utils/logger.py` — loguru 配置（参照 `bot/src/utils/logger.ts` 的分级模式）
